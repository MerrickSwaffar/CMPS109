...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if (file.fail()) {
      log << "get: " << header.filename << ": no such file" << endl;
      header.command = CIX_NAK;
      send_packet(client_sock, &header, sizeof header);
   } else {
      stringstream s;
      s << file.rdbuf();
      string get_output = s.str();
...
void reply_put(accepted_socket& client_sock, cix_header& header) {
    ...
    string put_output;
    if (file.fail()) {
       log << "put: cannot open file " << header.filename << endl;
       header.command = CIX_NAK;
       send_packet(client_sock, &header, sizeof header);
    } else {
       char buf[header.nbytes + 1];
       recv_packet(client_sock, buf, header.nbytes);
...
void reply_rm(accepted_socket& client_sock, cix_header& header) {
    ...
    if (deleted != 0) {
        log << "rm: remove failed: " << strerror (errno) << endl;
        header.command = CIX_NAK;
        header.nbytes = errno;
        send_packet (client_sock, &header, sizeof header);
    } else {
